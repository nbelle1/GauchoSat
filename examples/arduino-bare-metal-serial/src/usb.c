 ...
 * Original source: http://www.pjrc.com/teensy/usb_serial.html
 * Copyright (c) 2008, 2010, 2011 PJRC.COM, LLC
 */
/* Modified for ATmega32U4 Arduino Micro in the GauchoSat Project 
 * by Kpazawala Windross
 */
#include "usb.h"
...
#define STR_MANUFACTURER	L"atmega32u4"
...
// and group settings.
#define STR_SERIAL_NUMBER	L"000"
...
// is where you can make such changes.
...
#define NUM_DESC_LIST (sizeof(descriptor_list)/sizeof(struct descriptor_list_struct)) // Number of Descriptors
...
static uint8_t cdc_line_rtsdtr=0; //Tracks the state of RTS/DTR control signals sent by the host, also ignored but stored in RAM.
...
void usb_init(void){
	UHWCON = (1 << UVREGE);                // Power-On USB pads regulator
    USBCON = ((1<<USBE)|(1<<FRZCLK));		// enable USB
    PLLCSR = 0x12;				            // config PLL, 16 MHz xtal
